...
async function main() {
    ...
    Array.from(document.querySelector(".songlist").getElementsByTagName("li")).forEach(element => {
        element.addEventListener("click", e => {
            ...
            let trck = track.replaceAll(" ", "%20")
            snum = songs.indexOf(trck);
            playmusic(trck, element.getElementsByTagName("div")[0].getElementsByTagName("div")[0].innerHTML + "-" + element.getElementsByTagName("div")[0].getElementsByTagName("div")[1].innerHTML)
        })
    });
